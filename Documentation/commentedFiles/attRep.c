...
/**
 * @brief first attraction point id
 */
...
/**
 *  @brief last attraction point id
 */
...
/** @brief FOLLOW_OFFSET_X
 *  @details all in ENU frame
 */
#ifndef FOLLOW_OFFSET_X
#define FOLLOW_OFFSET_X 0.0f
#endif

/** @brief FOLLOW_OFFSET_Y
 *  @details all in ENU frame
 */
...
/** @brief gravity parameter
 * @details earth: 1.985f
 */
#ifndef GRAVITY
#define GRAVITY 1.985f
#endif

/** @brief comfy distance parameter */
#ifndef COMFY_DIST
#define COMFY_DIST 4.8f
#endif

/** @brief region size parameter */
#ifndef REGION_SIZE
#define REGION_SIZE 2.5f
#endif

/** @brief drone region size parameter */
#ifndef DRONE_REGION_SIZE
#define DRONE_REGION_SIZE 2.5f
#endif

/** @brief attraction multiplier parameter
 * @details ka = g * am
 */
#ifndef ATTRECTION_MULTIPLIER
#define ATTRECTION_MULTIPLIER 50.0f
#endif

/** @brief repulsion multiplier parameter
 * @details kb = g * rm
 */
#ifndef REPULSION_MULTIPLIER
#define REPULSION_MULTIPLIER 30.0f
#endif

/** @brief drone attraction multiplier parameter
 * @details kad = g * dam
 */
...
/** @brief drone repulsion multiplier parameter
 * @details kbd = g * drm
 */
...
/** @brief velocity limit parameter */
#ifndef VELOCITY_LIMIT
#define VELOCITY_LIMIT 5.0f
#endif

/** @brief struct to hold message goal information */
struct Message_Goal
{
  uint8_t wp_id;
...
/** @brief struct to hold debug message information */
...
/** @brief set point struct with given data
...
/** @brief attraction point */
static struct LlaCoor_f att_point = {0.0f, 0.0f, 0.0f};
/** @brief current position of drone */
 ...
 *
 * @param pos position of drone
 * @param vel velocity of drone
 * @param acc acceleration of drone
 * @param att_force attraction force of drone
 * @param member_force member attraction force of drone
 * @param rep_force repell force of drone
 */
...
/**
 * @brief Create a debug file object
 *
...
static int create_debug_file()
{
  ...
  if (mkdir("/home/finkensim/paparazzi/", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0 && errno != EEXIST)
    return -1;
  if (mkdir("/home/finkensim/paparazzi/logs", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0 && errno != EEXIST)
    return -1;
  if (mkdir("/home/finkensim/paparazzi/logs/ConSteer", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0 && errno != EEXIST)
    return -1;
...
static int create_log_file()
{
  ...
  if (mkdir("/home/finkensim/paparazzi/logs", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0 && errno != EEXIST)
    return -1;
  if (mkdir("/home/finkensim/paparazzi/logs/AttRep", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0 && errno != EEXIST)
    return -1;
  LOG_FILE = fopen(file_name, "w");
  if (LOG_FILE == NULL)
    printf("\nFile could not be opened!");
  else
    fprintf(LOG_FILE, "Startup-Time: %11.6f", get_sys_time_float());
  return 0;
}

/** @brief Get position in local ENU coordinates (float).
 * @param[in] ac_id aircraft id of aircraft info to get
 * @return position in local ENU coordinates from drone
 */
...
/**
 * @brief send acceleration information to paparazzi
 *
 * @param trans transmitter
 * @param dev device
 */
...
/**
 * @brief send goal information to paparazzi
 *
 * @param trans transmitter
 * @param dev device
 */
...
/**
 * @brief send attraction repulse information to paparazzi
 *
 * @param trans transmitter
 * @param dev device
 */
...
/**
 * @brief swarm initialization
 * @details register telemetry for paparazzi and create log files
 */
 ...
 * @return distance between two points in LlaCoor-f format
 */
...
static void updateSyncLinkMsg(struct LlaCoor_f *own_pos, uint8_t att_point_id)
{
  if (getDistance(own_pos, &att_point) <= 16.25f)
  {
    ...
    syncLink.reached = false;
}

/** @brief calculate attraction for drone to attraction points and other drones
 * @details attraction_force = ka/max(distance,0.0001) * dist_vec
 * @return force in EnuCoor_f
 */
...
/** @brief calculate repulsion for drone to repulsion points and other drones
...
/** @brief calculates total force out of attarction and repulsion
 * @details total_force = (kad*max(distance-d,0.0)/max(distance,1.0) - kbd*exp(-distance²/(dr²/log(kbd/kad)))) * dist_vec
 * @return force in EnuCoor_f
 */
 ...
 * @details updates the FOLLOW_WAYPOINT_ID  to a fixed offset from the last received location of other aircraft with id FOLLOW_AC_ID
 */
