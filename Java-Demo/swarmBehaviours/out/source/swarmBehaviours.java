/* autogenerated by Processing revision 1277 on 2021-10-24 */
...
class Drone {
   ...
   public void context_steering()
  {
    ...
    {
      PVector force = new PVector();
      force = this.contextMaps.get(GOALS).get(idx).mult(100).add(this.contextMaps.get(MEMBERS).get(idx)).add(this.contextMaps.get(DANGERS).get(idx).mult(300));
      ...
      { 
        ...
        if(cosSim < 0) force = force.mult(cosSim+1.5f);
      }
      forces.add(force);
    }
   ...
   public PVector limited_repulsion(PVector target)
  {
    ...
    if (d>(LIMIT-5)) strength = G * (5*log(d)-(LIMIT-1.5f))/3.85f;
    else {
   ...
   public PVector gausain_attraction(PVector target)
  {
    ...
    float strength = (G * tanh_gauss * d) / LIMIT - 0.5f;
    force.setMag(strength);
    return force;
  }
  
   public PVector inverse_gausain_attraction(PVector target)
  {
    final float LIMIT = 30;
    final float SIGMA = 5;
    final float MEAN = LIMIT/2;
    ...
    float d = force.mag();
    d = constrain(d, 0.5f, LIMIT);
    float tanh_inverted_gauss = (float)(Math.tanh(exp(sq(d-MEAN)/(2*sq(SIGMA)))/(SIGMA*sqrt(TWO_PI)))*10+0.75f);
